...
module UserCacheable
  ...
  DEFAULT_CACHE_EXPIRY = 12.hours

  def all_classrooms_cache(key:, groups: {}, expires_in: DEFAULT_CACHE_EXPIRY, &block)
    model_cache(last_updated_classroom, key: key, groups: groups, expires_in: expires_in, &block)
  end

  def classroom_cache(classroom, key:, groups: {}, expires_in: DEFAULT_CACHE_EXPIRY, &block)
    model_cache(classroom, key: key, groups: groups, expires_in: expires_in, &block)
  end

  def classroom_unit_cache(classroom_unit, key:, groups: {}, expires_in: DEFAULT_CACHE_EXPIRY, &block)
    model_cache(classroom_unit, key: key, groups: groups, expires_in: expires_in, &block)
  end

  def classroom_unit_by_ids_cache(classroom_id:, unit_id:, activity_id:, key:, groups: {}, expires_in: DEFAULT_CACHE_EXPIRY, &block)
  ...
  private def model_cache(object, key:, groups:, expires_in: DEFAULT_CACHE_EXPIRY, &block)
